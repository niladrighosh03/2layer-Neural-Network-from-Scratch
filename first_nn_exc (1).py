...
class FirstNN(object):
    ...
    def forword(self, X):
        ...
        # Second layer: Linear transformation + softmax activation
        z2=a1.dot(self.params['W2']) + self.params['b2']
        
        # Apply softmax to the second layer output
        exp_z2= np.exp(z2- np.max(z2, axis=1, keepdims=True))
        prob_scores = exp_z2 / np.sum(exp_z2, axis=1, keepdims=True)
    ...
    def loss(self, Y, prob_scores):
        ...
        #One hot encoding
        Y_one_hot = np.zeros((N, self.params['W2'].shape[1]))
        Y_one_hot[np.arange(N), Y] = 1
        
            # Cross-entropy loss
        log_probs = np.log(prob_scores + 1e-9)  # Adding epsilon to avoid log(0)
